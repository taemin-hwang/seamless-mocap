...
def calibrator:
    ...
    def initialize(self, cam_id):
        ...
        __zed.enable_positional_tracking(positional_tracking_parameters)
        ...
        __zed.enable_object_detection(obj_param)

        # Open the camera
        err = __zed.open(init_params)
    ...
    def run(self):
        ...
        image_size = __zed.get_camera_information().camera_resolution
        ...
        while True:
            # Grab an image
            if __zed.grab() == sl.ERROR_CODE.SUCCESS:
                __zed.retrieve_image(image, sl.VIEW.LEFT, sl.MEM.CPU, image_size)
                __zed.retrieve_image(depth_display, sl.VIEW.DEPTH)
                __zed.retrieve_measure(depth_map, sl.MEASURE.DEPTH)
                __zed.retrieve_objects(bodies, obj_runtime_param)

                image_left_ocv = image.get_data()[:,:,:3]
                cv2.imshow("__ZED | 2D View", image_left_ocv)
                cv2.imshow("__ZED | DEPTH View", depth_display)
